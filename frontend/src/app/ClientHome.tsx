...
const homeTranslations: Record<string, Record<string, string>> = {
  ...
  fr: {
    'home.discover_treatments': 'Découvrez nos soins exclusifs conçus pour sublimer votre beauté naturelle.',
...
export default function ClientHome({ content, settings, services, categories }: any) {
  ...
  const translateHome = (key: string, defaultValue: string) => {
    return homeTranslations[lang]?.[key] || defaultValue;
  };

  if (!content) {
    return (
      <div className="flex items-center justify-center min-h-screen text-stone-500 font-bold">
        {translateHome('home.loading', 'Cargando la web...')}
      </div>
    );
  }

  // 1. Traducir en caliente los servicios y categorías antes de renderizar
  ...
  // 2. Preparar todas las secciones disponibles
  const baseSections = [
    { id: 'hero', type: 'hero' },
    { id: 'about', type: 'about' },
    { id: 'cta', type: 'cta' }
  ...
  const catSections = translatedCategories.map((c: any) => ({
    id: c.id,
    type: 'category',
    data: c
  }));
  ...
  // 3. Aplicar el orden de la Base de Datos (Con defensa Ghost ID)
  ...
  // 4. Imponer la regla: Hero siempre primero
        ...
        {finalOrder.map((section, index) => {
          ...
          if (section.type === 'hero') {
            return (
              <section 
                key="hero" 
                className={`relative h-[100dvh] min-h-[600px] w-full flex snap-start snap-stop-always md:snap-none ${content.hero_alignment === 'top' ? 'items-start pt-48' : content.hero_alignment === 'bottom' ? 'items-end pb-32' : 'items-center'} ${content.hero_horizontal_alignment === 'left' ? 'justify-start' : content.hero_horizontal_alignment === 'right' ? 'justify-end' : 'justify-center'} p-6 md:p-12 overflow-hidden mt-0`}
              >
              ...
              <section key="about" className="bg-[#F5F2EE] relative flex items-start h-[100dvh] snap-start snap-stop-always md:h-auto md:snap-none pt-20 md:pt-24 overflow-hidden">
                ...
                <div className="about-container max-w-7xl mx-auto px-6 flex flex-col md:grid md:grid-cols-2 gap-10 md:gap-16 items-center w-full h-full pb-16">
                  <div className="space-y-4 md:space-y-8 flex flex-col items-center text-center md:items-start md:text-left flex-shrink-0">
                    <h2 className="about-title text-3xl md:text-5xl font-serif font-extrabold text-stone-900 leading-tight">
          ...
          if (section.type === 'category') {
            ...
            const categoryServices = translatedServices.filter((s: any) => s.category_id === category.id && s.is_active);
                ...
                <div className="w-full max-w-7xl mx-auto px-6 mb-6 flex-shrink-0 flex flex-col md:flex-row md:justify-between md:items-end gap-3 md:gap-8">
                  <div className="max-w-2xl">
                    ...
                    <p className="hidden md:block text-lg md:text-xl text-stone-500 mt-2">
                      {category.description || translateHome('home.discover_treatments', 'Descubre nuestros tratamientos exclusivos diseñados para resaltar tu belleza natural.')}
                    </p>
                  </div>
                  <Link href={`/tratamientos/${category.slug || category.id}`} className="self-end md:self-auto inline-flex items-center gap-2 font-bold text-[#d4af37] hover:text-stone-900 transition-colors uppercase tracking-widest text-[10px] md:text-sm">
                    <span className="hidden md:inline">{translateHome('home.see_catalog', 'Ver Catálogo')}</span>
                    <span className="md:hidden">{translateHome('home.see_all', 'Ver todo')}</span>
