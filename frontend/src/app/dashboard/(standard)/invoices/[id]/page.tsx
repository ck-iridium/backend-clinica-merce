...
export default function InvoicePreviewPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error(e);
      alert(t('dashboard.invoices.error_loading_invoice') || "Error cargando la vista de factura");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
        <div className="text-center py-32">
          ...
          <p className="text-stone-500 font-medium tracking-widest uppercase text-xs">
            {t('dashboard.invoices.loading_sheet') || 'Cargando folio...'}
          </p>
        </div>
    );
  }
  ...
  if (!invoice) return (
    <div className="p-10 text-center font-bold text-stone-500">
      {t('dashboard.invoices.doc_not_available') || 'Documento no disponible.'}
    </div>
  );
        ...
        <div className="xl:col-span-1 space-y-6 print:hidden">
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-stone-100 shadow-sm">
            <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-4">
              {t('dashboard.invoices.client_info') || 'Información del Cliente'}
            </p>
            <div className="flex flex-col gap-1 mb-4">
              <span className="font-extrabold text-stone-800 text-lg leading-tight">{client?.name || (t('dashboard.invoices.deleted_client') || 'Cliente Borrado')}</span>
              <span className="text-sm font-medium text-stone-500">{client?.email || (t('dashboard.invoices.no_email') || 'Sin email')}</span>
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-stone-100 shadow-sm">
            <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-4">
              {t('dashboard.invoices.state_control') || 'Control de Estado'}
            </p>
            ...
            <Dialog open={showConfirmModal} onOpenChange={setShowConfirmModal}>
              <DialogTrigger asChild>
                ...
                >
                  <span>{invoice.status === 'paid' ? (t('dashboard.invoices.paid_state') || '✔ Estado: Pagado') : (t('dashboard.invoices.pending_state') || '⏳ Estado: Pendiente')}</span>
                  <span className="text-xs opacity-70 italic underline">{t('dashboard.invoices.change_state') || 'Cambiar'}</span>
                </button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-[425px] p-0 overflow-hidden bg-white/95 backdrop-blur-xl border-white/20 shadow-2xl rounded-[2rem] flex flex-col max-h-[85dvh]">
                <DialogHeader className="p-8 pb-4 bg-stone-50/50 border-b border-stone-100">
                  ...
                  <DialogTitle className="text-2xl font-serif font-bold text-stone-900 tracking-tight">
                    {t('dashboard.invoices.confirm_state_title') || 'Cambiar Estado'}
                  </DialogTitle>
                  <DialogDescription className="text-stone-500 font-medium">
                    {t('dashboard.invoices.confirm_state_desc') || '¿Estás seguro de que deseas cambiar el estado de esta factura?'}
                  </DialogDescription>
                </DialogHeader>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-4">
                  <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100">
                    <p className="text-sm font-bold text-amber-700 flex gap-2">
                      <span>⚠️</span>
                      {t('dashboard.invoices.accounting_warning') || 'Esta acción afectará directamente a los reportes de contabilidad y reconciliación de caja.'}
                    </p>
                  </div>
                </div>
                ...
                <DialogFooter className="p-8 pt-4 bg-stone-50/50 border-t border-stone-100 flex gap-3">
                  ...
                  >
                    {t('dashboard.invoices.confirm_change') || 'Confirmar Cambio'}
                  </button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
            ...
            >
              ...
              {t('dashboard.invoices.print_document') || 'IMPRIMIR DOCUMENTO'}
            </button>
          </div>
            ...
            <div className="p-[20mm] relative z-10 w-full h-full flex flex-col">
                ...
                <div className="flex justify-between items-start mb-8 border-b border-stone-200 pb-4">
                  <div className="flex flex-col">
                    ...
                    {settings?.logo_pdf_b64 ? (
                       ...
                       <div className="w-24 h-24 bg-stone-100 rounded-2xl mb-4 flex items-center justify-center text-stone-300 font-bold text-[10px] text-center p-2 border border-stone-200 border-dashed">
                         {t('dashboard.invoices.no_logo') || 'Sin Logo'}
                       </div>
                    )}
                  ...
                  <div className="text-right flex flex-col items-end">
                    <h1 className="text-4xl font-extrabold text-stone-200 tracking-tighter mb-4">
                    ...
                    <div className="text-xs text-stone-500 font-medium grid grid-cols-2 gap-x-4 gap-y-1 text-right w-full max-w-[200px]">
                      ...
                      <span className={`font-bold ${invoice.status === 'paid' ? 'text-emerald-500' : 'text-orange-500'}`}>
                        {invoice.status === 'paid' ? (t('dashboard.invoices.paid') || 'Pagado') : (t('dashboard.invoices.pending') || 'Pendiente')}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {!invoice.is_simplified && (
                  <div className="mb-6">
                    <p className="text-[10px] font-bold text-[#d4af37] uppercase tracking-widest mb-3 border-b border-stone-100 pb-2">
                      {t('dashboard.invoices.bill_to') || 'Facturar o Cobrar A:'}
                    </p>
                    <div className="pl-2">
                      <p className="text-base font-bold text-stone-800 mb-1">{client?.name || (t('dashboard.invoices.no_name_client') || 'Cliente sin nombre')}</p>
                      ...
                      {client?.dni && (
                        <p className="text-xs text-stone-600"><strong>{t('dashboard.invoices.nif_cif') || 'NIF/CIF:'}</strong> {client.dni}</p>
                      )}
                      {client?.address && (
                        <p className="text-xs text-stone-600"><strong>{t('dashboard.invoices.address') || 'Dirección:'}</strong> {client.address}</p>
                      )}
                    </div>
                  </div>
                )}
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b-2 border-stone-800 text-xs uppercase tracking-widest text-stone-800">
                        <th className="py-4 font-extrabold w-3/4">{t('dashboard.invoices.description_concept') || 'Descripción / Concepto'}</th>
                        <th className="py-4 font-extrabold text-right">{t('dashboard.invoices.net_amount') || 'Importe Neto'}</th>
                      </tr>
                    </thead>
                ...
                <div className="flex justify-end mb-8">
                  <div className="w-[250px]">
                     <div className="flex justify-between py-2 text-sm text-stone-500">
                       <span>{t('dashboard.invoices.taxable_base') || 'Base Imponible'}</span>
                     ...
                     <div className="flex justify-between items-end pt-2">
                       <span className="font-extrabold text-stone-800">{t('dashboard.invoices.total') || 'TOTAL'}</span>
                ...
                <div className="mt-auto border-t border-stone-200 pt-4 flex justify-between items-end">
                   <div className="max-w-[300px]">
                     <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-2">
                       {t('dashboard.invoices.instructions') || 'Instrucciones'}
                     </p>
                     <p className="text-xs text-stone-500 text-left">
                       {t('dashboard.invoices.instructions_desc') || 'Los bonos y tratamientos adquiridos están sujetos a su periodo de vigencia. El abono se ha registrado correctamente en nuestra contabilidad interna.'}
                     </p>
                   </div>
                   <div className="text-right flex flex-col items-end relative min-w-[200px] min-h-[100px]">
                     ...
                     <p className="text-xs text-stone-400 italic mt-2">
                       {t('dashboard.invoices.thanks_confidence') || 'Gracias por su confianza.'}
                     </p>
