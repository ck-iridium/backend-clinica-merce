        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-base font-bold text-stone-900">
              {t('dashboard.branding.footer_logo.title') || 'Logotipo del Pie de Página (Footer)'}
            </h4>
            <p className="text-[10px] text-stone-400 font-bold uppercase tracking-wider mt-0.5">
              {t('dashboard.branding.footer_logo.subtitle') || 'Logotipo exclusivo opcional y contraste sobre fondo oscuro'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {settings.logo_footer_b64 ? (
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-50 border border-amber-200/60 text-[11px] font-bold text-amber-800">
              ...
              {t('dashboard.branding.footer_logo.current_logo') || 'Logotipo exclusivo activo'}
            </span>
          ) : (
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-stone-100 text-[11px] font-bold text-stone-600">
              {t('dashboard.branding.footer_logo.default_inherited') || 'Heredando de cabecera'}
            </span>
          )}
        </div>
      </div>
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="h-44 bg-[#121212] border border-stone-800 rounded-2xl flex flex-col items-center justify-center p-6 relative group/footerlogo overflow-hidden">
            <span className="absolute top-3 left-4 text-[10px] uppercase font-bold tracking-widest text-stone-500">
              {t('dashboard.branding.footer_logo.current_logo') || 'Simulación Fondo Footer'}
            </span>
            ...
            {effectiveFooterLogo ? (
              <div className="relative group/box flex items-center justify-center w-full h-full pt-4">
                ...
                {settings.logo_footer_b64 && (
                  <button
                    id="branding-footer-logo-delete-btn"
                    type="button"
                    ...
                    title={t('dashboard.branding.footer_logo.delete_btn') || 'Eliminar Logotipo del Footer (volver al de cabecera)'}
                  >
                    <Trash2 size={14} />
                  </button>
                )}
              ...
              <div className="flex flex-col items-center gap-2 text-stone-600">
                ...
                <span className="text-xs font-bold text-stone-500">
                  {t('dashboard.branding.desktop_logo.no_logo') || 'Sin logotipo'}
                </span>
              </div>
            )}
          </div>
        ...
        <div className="lg:col-span-7 space-y-5">
          <div className="p-5 bg-stone-50/80 rounded-2xl border border-stone-200/60 space-y-4">
            <span className="text-xs font-bold text-stone-700 uppercase tracking-wider block">
              {t('dashboard.branding.footer_logo.color_mode_label') || 'Modo de Color en el Pie de Página'}
            </span>
            <p className="text-xs text-stone-500 leading-relaxed">
              {t('dashboard.branding.footer_logo.subtitle') || 'Dado que el pie de página de la clínica cuenta con un elegante fondo antracita oscuro, puedes forzar la conversión del logotipo a blanco puro o mantener sus colores originales.'}
            </p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button
                id="branding-footer-mode-white-btn"
                type="button"
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-black">
                    {t('dashboard.branding.footer_logo.mode_white') || 'Blanco Puro (Invertido)'}
                  </span>
                  {footerLogoMode === 'white' && (
                ...
                <span className="text-[11px] text-stone-500 block leading-tight font-normal">
                  {t('dashboard.branding.footer_logo.mode_white_desc') || 'Recomendado. Convierte logotipos negros u oscuros a blanco nítido para máxima legibilidad.'}
                </span>
              </button>

              <button
                id="branding-footer-mode-original-btn"
                type="button"
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-black">
                    {t('dashboard.branding.footer_logo.mode_original') || 'Color Original'}
                  </span>
                  {footerLogoMode === 'original' && (
                    <Check size={14} className="text-[#d4af37]" />
                  )}
                ...
                <span className="text-[11px] text-stone-500 block leading-tight font-normal">
                  {t('dashboard.branding.footer_logo.mode_original_desc') || 'Muestra el archivo con sus colores nativos de marca sin aplicar filtros.'}
                </span>
              </button>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3 p-4 bg-amber-50/70 rounded-2xl border border-amber-200/50 text-xs text-amber-900 leading-relaxed">
            ...
            <span>
              {t('dashboard.branding.footer_logo.info_tip') || 'Comportamiento inteligente: Si no se sube un logotipo específico para el pie de página, el sistema usará automáticamente el de la cabecera. Es totalmente opcional.'}
            </span>
          </div>
        </div>
