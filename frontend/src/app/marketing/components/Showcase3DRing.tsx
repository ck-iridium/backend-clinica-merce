...
export default function Showcase3DRing({
  ...
  return (
    <section id="sectors" className="h-screen min-h-[720px] bg-stone-50/50 border-y border-stone-100 overflow-hidden relative flex flex-col justify-between py-12 md:py-16 select-none">
      
      {/* Cabecera Editorial */}
      <div className="w-full max-w-7xl mx-auto px-6 text-center shrink-0">
        <span className="text-blue-600 text-[10px] font-black uppercase tracking-[0.25em] block mb-2">Especialidades</span>
        <h2 className="text-3xl md:text-5xl font-serif font-semibold tracking-tight text-stone-950">
          Sectores de Alta Gama
        </h2>
        <p className="text-stone-500 text-xs md:text-sm mt-2 font-medium">
          Navega por las especialidades de ProBookia con nuestro carrusel 3D Coverflow infinito.
        </p>
      </div>

      {/* Contenedor central del escenario Coverflow 3D */}
      <div className="relative w-full flex-1 flex items-center justify-center my-4 [perspective:1000px] [transform-style:preserve-3d]">
        
        {/* Contenedor de tarjetas - Perfectamente centrado */}
        <div className="relative w-[280px] h-[440px] [transform-style:preserve-3d] select-none">
          {sectorsToRender.map((sector, index) => {
            ...
            // Transformaciones Coverflow dinámicas según el offset (La central offset===0 es más grande)
            let transformStr = "";
            let zIndex = 0;
            let opacity = 1;
            ...
            if (offset === 0) {
              ...
              transformStr = "translateX(65%) scale(0.8) rotateY(-15deg)";
              zIndex = 20;
              opacity = 0.8;
            } else if (offset === -1) {
              transformStr = "translateX(-65%) scale(0.8) rotateY(15deg)";
              ...
              transformStr = "translateX(120%) scale(0.65) rotateY(-25deg)";
              zIndex = 10;
              opacity = 0.4;
            } else if (offset === -2) {
              transformStr = "translateX(-120%) scale(0.65) rotateY(25deg)";
            ...
            return (
              <div
                ...
                className={`absolute inset-0 cursor-pointer [backface-visibility:hidden] select-none ${
                  isActive 
                    ? 'drop-shadow-[0_30px_60px_rgba(28,25,23,0.18)]' 
      ...
      {/* LA FICHA BLANCA EMERGENTE - Absoluta a la derecha en la esquina inferior para no tapar la card central */}
      {sectorsToRender[realIndex] && (
        <div className={`absolute bottom-6 right-6 md:bottom-12 md:right-12 max-w-sm w-[90%] md:w-[350px] bg-white/95 backdrop-blur-md p-6 rounded-2xl shadow-2xl border border-stone-100/80 transition-all duration-500 transform z-30 ${
