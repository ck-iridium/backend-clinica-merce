...
export default function TreatmentMedia({ imageUrl, videoUrl, headerStyle }: TreatmentMediaProps) {
  ...
  if (headerStyle === 'split_video' && videoUrl) {
    return (
      <div className="relative w-full h-full rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-[0_20px_40px_-15px_rgba(0,0,0,0.2)] bg-stone-100">
        {!hasError ? (
          <video
            ref={videoRef}
            src={videoUrl}
  ...
  return (
    <div className={`relative ${headerStyle === 'split_video' ? 'w-full h-full rounded-[2rem] md:rounded-[3rem] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.2)]' : 'w-full h-full'} overflow-hidden bg-stone-100`}>
      {imageUrl && !hasError ? (
        <img
          ref={imgRef}
          src={imageUrl}
